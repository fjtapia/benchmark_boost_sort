...
{
	assert ( dest.size() >= src.size());
	Iter2_t it_aux = util::move_backward ( dest.first+ src.size(),
			                                    src.first, src.last);
...
*/
    ...
    Value_t * ptr_aux = util::merge_construct (src1.first, src1.last,
...
*/
	return  util::merge_uncontiguous (src1.first, src1.last,
			                          src2.first, src2.last,
			                          aux.first, comp);
                 ...
                 range< Iter1_t > rng2, Compare cmp)
{
    ...
    if (rbx.first == rbx.last) init_move (rbuf, rng2);
    else  half_merge (rbuf, rx2, rbx, cmp);
};
