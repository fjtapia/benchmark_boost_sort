...
struct merge_block
{
	...
	//                          CONSTANTS
	//------------------------------------------------------------------------
	static const size_t BLOCK_SIZE = (size_t)1 << Power2;
	static const size_t LOG_BLOCK = Power2 ;
	  ...
	  owned(pcirc_buffer== nullptr)
    {
        ...
        for (size_t i = 0; i < nblock; ++i) index.emplace_back (i);
    		              ...
    		              it_index itx_last)
    {
    	...
    	{	//----------------------------------------------------------------
    		...
    		if ( ptr_circ->size() == 0)
    		{
    			...
    			{	if ( not is_tail(*itxB)) *(itx_out++) = *itxB;
    	...
    	{	// the index A is finished
    		...
    		while ( itxB != indexB.end()) *(itx_out++) = *(itxB++);
    	}
    	else
    	{	// The list B is finished
    		...
    		while ( itxA != indexA.end()) *(itx_out++) = *(itxA++);
    	};
    };
/*
    void merge_range_pos (it_index itx_first, it_index itx_mid,
    		              it_index itx_last)
    {
    	...
    	it_index itx_out = itx_first;
    	it_index itxA = indexA.begin(), itxB = indexB.begin() ;
    	range_it rngA = get_range ( *itxA), rngB = get_range( *itxB);
    	Iter_t itA = rngA.first, itB = rngB.first ;
    	...
    	while ( itxA != indexA.end() and itxB != indexB.end())
    	{
    		...
    		bool side = util::merge_circular (itA, rngA.last,
    				                          itB, rngB.last,
											  circ, cmp, itA, itB);
    		if ( side)
    		{	// rngA is finished
    			circ.pop_move_front ( rngA.first, rngA.size());
    			*(itx_out++) = *(itxA++);
    			if ( itxA != indexA.end())
    			{	rngA = get_range( *itxA);
    				itA = rngA.first;
    		...
    		{	// rngB is finished
    			if ( not is_tail ( *itxB))
    			{	circ.pop_move_front ( rngB.first, rngB.size());
    				*(itx_out++) = *itxB;
    			};
    			++itxB ;
    			if ( itxB != indexB.end())
    	...
    	{	// the index A is finished
    		rngB = get_range ( *itxB);
    		circ.pop_move_front ( rngB.first, circ.size());
    		while ( itxB != indexB.end()) *(itx_out++) = *(itxB++);
    	}
    	else
    	{	// The list B is finished
    		...
    		rngA = get_range( *itxA);
    		circ.pop_move_front ( rngA.first, circ.size());
    		while ( itxA != indexA.end()) *(itx_out++) = *(itxA++);
    	};
    };
*/
    ...
    {	//--------------------------------------------------------------------
		...
		while (pos_ini < nelem)
		{
			while (pos_ini < nelem and index[pos_ini] == pos_ini) ++pos_ini;
