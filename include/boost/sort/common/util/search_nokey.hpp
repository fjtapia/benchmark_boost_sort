                         ...
                         const Compare & comp = Compare())
{
    ...
    {
        it_out = LI + ((LS - LI) >> 1);
        if (comp(*it_out, val))  LI = it_out + 1;
        else                     LS = it_out;
    };
    return LS;
};
                         ...
                         const Compare &comp = Compare())
{
    ...
    {
        it_out = LI + ((LS - LI + 1) >> 1);
        if (comp(val, *it_out)) LS = it_out - 1;
        else                    LI = it_out;
    };
    return LS;
};
                          ...
                          Compare comp = Compare())
{
    ...
    Iter_t LS = internal::find_first(first, last, val, comp);
...
template<class Iter_t, class Compare = compare_iter<Iter_t> >
inline Iter_t find_last(Iter_t first, Iter_t last,
                        const value_iter<Iter_t> &val, Compare comp = Compare())
{
    assert((last - first) >= 0);
    if (last == first) return last;
    Iter_t LS = internal::find_last(first, last, val, comp);
    return (comp(*LS, val) or comp(val, *LS)) ? last : LS;
};
                          ...
                          Compare &comp = Compare())
{
    assert((last - first) >= 0);
    if (last == first) return last;
    Iter_t itaux = internal::find_first(first, last, val, comp);
...
inline Iter_t upper_bound(Iter_t first, Iter_t last,
                          const value_iter<Iter_t> &val, Compare &comp =
                                          Compare())
{
    assert((last - first) >= 0);
    if (last == first) return last;
    Iter_t itaux = internal::find_last(first, last, val, comp);
