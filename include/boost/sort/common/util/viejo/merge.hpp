...
//-----------------------------------------------------------------------------
template < class Iter1_t, class Iter2_t, class Iter3_t, class Compare >
Iter3_t merge (Iter1_t buf1, const Iter1_t end_buf1, Iter2_t buf2,
...
Value_t *merge_construct (Iter1_t first1, const Iter1_t last1, Iter2_t first2,
                          const Iter2_t last2, Value_t *it_out, Compare comp)
{
    //-------------------------------------------------------------------------
    //                       Metaprogramming
    //------------------------------------------------------------------------- 
...
//---------------------------------------------------------------------------
template < class Iter1_t, class Iter2_t, class Compare >
Iter2_t merge_half_backward (Iter1_t buf1,  Iter1_t end_buf1, Iter2_t buf2,
                     Iter2_t end_buf2, Iter1_t end_buf_out, Compare comp)
{
    //-------------------------------------------------------------------------
    //                         Metaprogramming
    //-------------------------------------------------------------------------
    ...
    if ( buf1 == end_buf1 ) return move_backward (end_buf_out, buf2 , end_buf2);

    if ( not comp( *buf2, *(end_buf1 - 1)))
    {   move_backward (end_buf_out, buf2 , end_buf2);
        return buf1 ;
    };

    if ( not comp ( *buf1, *(end_buf2 - 1)))
    {   Iter1_t mid = move_backward( end_buf_out, buf1, end_buf1);
        return move_backward ( mid, buf2, end_buf2);
    };

    while ((buf1 != end_buf1) and (buf2 != end_buf2))
    {   *(--end_buf_out) = (not comp (*(end_buf2 -1), *(end_buf1 -1))) ?
    		                std::move (*(--end_buf2)):
                            std::move (*(--end_buf1))  ;
    };
    return (buf1 == end_buf1) ? move_backward (end_buf_out, buf2, end_buf2) : buf1;
};
...
bool merge_circular  (Iter1_t buf1, Iter1_t end_buf1,
                      Iter2_t buf2, Iter2_t end_buf2,
                      Circular &circ, Compare comp,
					  Iter1_t &it1_out, Iter2_t &it2_out )
{
    ...
    bool sw = ( buf1 != end_buf1 and buf2 != end_buf2), side = true;
    while ( sw)
    {
        circ.push_back ((side = comp (*buf2, *buf1))? std::move (*(buf2++)):
                                      std::move (*(buf1++)) );
