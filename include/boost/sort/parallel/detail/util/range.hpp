...
#include <boost/sort/spinsort/util/range.hpp>
#include <cassert>
#include <functional>
#include <iterator>
#include <memory>
...
//-----------------------------------------------------------------------------
//  function : code_stable_sort
/// @brief : check if the elements in the range are sorted, reverse sorted or none
/// @param rng : range to examine
/// @return 0 : unsorted 1: sorted 2: reverse_sorted
/// Two consecutive equal elements can't be reverse sorted
//-----------------------------------------------------------------------------
template < class Iter_t,class Compare >
inline uint32_t code_stable_sort( const range<Iter_t> & rng, Compare comp)
{
	if (rng.size() < 2 ) return 1 ;
    bool sw = true;
    Iter_t it1 = rng.first, it2 = rng.first + 1;
    while ( it2 != rng.last and (sw = not comp (*it2, *it1)))
    	it1 = it2++;
    if( sw) return 1 ;
    if ( it1 != rng.first ) return 0 ;
    sw = true ;
    while (it2 != rng.last and (sw = comp (*it2, *it1)))
    	it1 = it2++;
    return (sw) ? 2 :0 ;
};
//-----------------------------------------------------------------------------
//  function : code_non_stable_sort
/// @brief : check if the elements in the range are sorted, reverse sorted or none
/// @param rng : range to examine
/// @return 0 : unsorted 1: sorted 2: reverse_sorted
/// Two consecutive equal elements can be reverse sorted
//-----------------------------------------------------------------------------
template < class Iter_t,class Compare >
inline uint32_t code_non_stable_sort( const range<Iter_t> & rng, Compare comp)
{
	if (rng.size() < 2 ) return 1 ;
    bool sw = true;
    Iter_t it1 = rng.first, it2 = rng.first + 1;
    ...
    if ( it1 != rng.first ) return 0 ;
    sw = true ;
    while (it2 != rng.last and (sw = not comp (*it1, *it2)))
    	it1 = it2++;
    return (sw) ? 2 :0 ;
};
...
//-----------------------------------------------------------------------------
//  function : is_sorted
/// @brief : check if the elements in the range are sorted
/// @param rng : range to examine
/// @return true : sorted , false :unsorted
//-----------------------------------------------------------------------------
template < class Iter_t,class Compare >
inline bool is_sorted ( const range<Iter_t> & rng, Compare comp)
{
    //if (rng.size() < 2 ) return true ;
    bool sw = true;
    for (Iter_t it1 = rng.first, it2 = rng.first + 1;
         it2 != rng.last and (sw = not comp (*it2, *it1)); it1 = it2++);
    return sw ;
};
//-----------------------------------------------------------------------------
//  function : is_reverse_sorted
/// @brief : check if the elements in the range are reverse sorted
/// @param rng : range to examine
/// @return true : sorted , false :unsorted
//-----------------------------------------------------------------------------
template < class Iter_t,class Compare >
inline bool is_reverse_sorted ( const range<Iter_t> & rng, Compare comp)
{
    if (rng.size() < 2 ) return true ;
    bool sw = true;
    for (Iter_t it1 = rng.first, it2 = rng.first + 1;
         it2 != rng.last and (sw = comp (*it2, *it1)); it1 = it2++);
    return sw ;
};
